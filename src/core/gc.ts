...
/**
 * `vedit gc [--yes]` (roadmap "cache 剪定"): HANDOFF §5 names this an
 * explicit known gap — "プロキシ/キャッシュの削除戦略がない(cache/ は増える
 * 一方)". This lists (default) or deletes (`--yes`) cache/ files no longer
 * referenced by the CURRENT manifest, plus orphan transcripts (a source
 * removed from the timeline entirely leaves its transcript-<sourceId>.json
 * behind at the project root — that's the one non-cache/ category this also
 * covers, per the roadmap item's spec).
 *
 * Protected, never listed as an orphan regardless of manifest content:
 * `cache/fonts.json` (system font list cache), `cache/export-results.json`
 * (the export-results card record), anything mid-write (`*.tmp-*` — a
 * concurrent write's temp file, not garbage). revisions.jsonl/
 * revisions.jsonl.bak live at the project ROOT, not cache/, so `vedit
 * compact`'s backup is untouched by construction, not by a special case
 * here.
 */
export interface GcOrphan {
  /** Relative to the project directory (e.g. "cache/proxy-s1.mp4" or "transcript-s1.json"). */
  path: string;
  bytes: number;
  reason: string;
}
...
export interface GcResult {
  orphans: GcOrphan[];
  totalBytes: number;
  /** True only after a real (--yes) run actually deleted the listed files. */
  deleted: boolean;
}

function classifyCacheOrphan(name: string): string {
  if (name.startsWith('proxy-')) return 'orphan proxy (no source references it)';
  if (name.startsWith('peaks-')) return 'orphan waveform (no source references it)';
  if (name.startsWith('sc-')) return 'orphan scene thumbnail';
  if (name.startsWith('thumb-')) return 'orphan source thumbnail (source removed)';
...
export async function planGc(project: Project): Promise<GcResult> {
  const m = await project.manifest();
  const liveSourceIds = new Set(m.sources.map((s) => s.id));

  const protectedRel = new Set<string>(['cache/fonts.json', 'cache/export-results.json']);
  for (const s of m.sources) {
    if (s.proxy) protectedRel.add(s.proxy);
    if (s.peaks) protectedRel.add(s.peaks);
  ...
  for (const id of liveSourceIds) {
    ...
    for (const scene of sceneFile.scenes) {
      if (scene.thumb) protectedRel.add(scene.thumb);
    }
  }
  ...
  for (const name of cacheEntries) {
    if (name.includes('.tmp-')) continue; // in-flight / crash-remnant write — never gc's business
    const rel = `cache/${name}`;
    if (protectedRel.has(rel)) continue;
    const abs = path.join(project.cacheDir, name);
    const st = await fs.stat(abs).catch(() => null);
    if (!st || !st.isFile()) continue; // skip subdirectories defensively
...
export async function runGc(project: Project, opts: { yes?: boolean } = {}): Promise<GcResult> {
  const plan = await planGc(project);
  if (!opts.yes) return plan;
