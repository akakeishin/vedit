...
function assertFilter(name: string, purpose: string): void {
  if (!ffmpegHasFilter(name)) {
    throw new Error(
      `this ffmpeg build lacks the \`${name}\` filter (needed for ${purpose}). Install \`brew install ffmpeg-full\` or set VEDIT_FFMPEG.`,
    );
  }
}
...
export type ColorAdjustEntry = NonNullable<Manifest['colorAdjust']>[string];
 ...
 */
export function buildColorChain(colorTransform: Source['colorTransform'], adjust?: ColorAdjustEntry): string {
  const parts: string[] = [];
  if (colorTransform && colorTransform.type !== 'none') {
    if (colorTransform.type === 'hlg') {
      ...
      if (!colorTransform.lut) throw new Error('color transform type "lut" requires a lut path');
...
// ---- W5: color-match — read-only shot-matching proposal (signalstats) ----
 ...
 */
export function parseSignalStats(text: string): SignalStats {
  const grab = (key: string): number => {
    const m = text.match(new RegExp(`lavfi\\.signalstats\\.${key}=([\\-0-9.]+)`));
    if (!m) throw new Error(`signalstats output missing ${key}`);
    return Number(m[1]);
  };
...
export interface ColorMatchProposal {
  ...
  suggested: { exposure: number; wb: number; sat: number };
}
 ...
 */
export function suggestColorAdjust(base: SignalStats, target: SignalStats): { exposure: number; wb: number; sat: number } {
 ...
 */
export async function proposeColorMatch(
  m: Manifest,
  projectDir: string,
  baseSourceId: string,
  targetSourceIds: string[],
): Promise<{ base: { sourceId: string } & SignalStats; proposals: ColorMatchProposal[] }> {
  const srcById = new Map(m.sources.map((s) => [s.id, s]));
  const sample = async (sourceId: string): Promise<SignalStats> => {
  ...
  const proposals: ColorMatchProposal[] = [];
