...
export async function makeProxy(file: string, outPath: string, p: ProbeResult): Promise<void> {
  ...
  const fps = Math.min(30, Math.round(p.fps) || 30);
  await run('ffmpeg', [
    '-y', '-i', file,
    '-vf', `scale=-2:${targetH}`,
    '-r', String(fps),
    '-c:v', 'libx264', '-preset', 'veryfast', '-crf', '23',
    '-g', String(fps), // 1s keyframe interval → snappy seeks
    '-pix_fmt', 'yuv420p',
...
export async function transcribe(
  ...
  return { sourceId, language: j.result?.language ?? opts.language ?? 'auto', words };
}
...
export async function ingestFile(
  ...
  await makeProxy(abs, path.join(project.dir, proxyRel), p);
  if (p.hasAudio) {
    notify('extracting waveform');
    await makePeaks(abs, path.join(project.dir, peaksRel));
  }
  const source: Source = {
    id,
    path: abs,
  ...
  if (p.hasAudio && opts.transcribe !== false) {
    ...
    const t = await transcribe(abs, id, { language: opts.language });
