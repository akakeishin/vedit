...
[![CI](https://github.com/akakeishin/vedit/actions/workflows/ci.yml/badge.svg)](https://github.com/akakeishin/vedit/actions/workflows/ci.yml)
...
> [!IMPORTANT]
> 現在は技術プレビューです。macOSで検証済みです。Windows / LinuxはCI上の
> コアテスト以外、実素材・ブラウザUI・書き出しをまだ実機検証していません。
...
- Node.js 20以上
- `ffmpeg` / `ffprobe`
- Chrome、Safariなどのモダンブラウザ
- 文字起こしを使う場合だけwhisper.cppモデル（`vedit doctor`から導入可能）

macOSでは、Node.jsとffmpegをHomebrewで用意できます。

```bash
brew install node ffmpeg
```

GitHubからCLIとWeb NLEをグローバルインストールします。npm registryへの公開は
...
Codex / Claude Codeから会話で利用する場合は、同梱Skillも導入します。既存の
`vedit` Skillを勝手に上書きせず、衝突時は停止して案内します。

```bash
vedit-install-skills          # Codex + Claude Code
# または片方だけ:
vedit-install-skills --codex
vedit-install-skills --claude
```

更新も同じコマンドです。Skillはインストール済みパッケージへのリンクなので、
通常は再導入不要です。
...
npm run build
npm link
vedit-install-skills
```

セットアップに問題がある場合は `vedit doctor` を実行してください。ffmpegの
`drawtext` / `ass` が不足する環境では、コマンドが必要な構成を案内します。
...
```bash
# 発話中心の動画を文字起こしする場合だけ初回にモデルを導入（約1.6GB）
vedit doctor --download-model ggml-large-v3-turbo

vedit create ~/edits/myvlog --name myvlog
vedit open --project ~/edits/myvlog  # → 素材がまだ0件でも、まずアプリを表示
vedit ingest ~/Movies/raw.mp4 --project ~/edits/myvlog
cd ~/edits/myvlog
...
- **Codex**: `vedit-install-skills --codex` で `~/.codex/skills/vedit` に導入。
  このリポジトリ内では `.agents/skills/vedit/SKILL.md` からも検出されます。
- **Claude Code**: `vedit-install-skills --claude` で
  `~/.claude/skills/vedit` に導入。
...
変更提案はIssueまたはPull Requestで受け付けます。開発手順は
[CONTRIBUTING.md](CONTRIBUTING.md)を参照してください。脆弱性は公開Issueへ動画や
トークンを添付せず、[Security policy](SECURITY.md)に従って報告してください。
