...
import { COLOR_WARNING_MESSAGE, cullingStats, needsColorTransform, orphanedOverlays, orphanedSprites, overlayGeometryWarnings, segments, timelineDuration, } from '../core/ops.js';
import { captionCues } from '../core/captions.js';
import { adaptiveThreshold } from '../core/detect.js';
...
function issueList(category) {
    const issues = [];
    const push = (severity, message, tlTime) => {
 ...
 */
export function checkPendingQueues(m, sceneFiles, candidates) {
 ...
 */
export function checkOverlayGeometry(m) {
    const { issues, push } = issueList('overlay-geometry');
    for (const w of overlayGeometryWarnings(m))
        push('warning', w);
    return issues;
}
/**
 ...
 */
export function checkCaptionCues(cues, maxCps) {
    const { issues, push } = issueList('captions');
    const clip = (s, n = 20) => (s.length > n ? `${s.slice(0, n)}…` : s);
...
export function checkColorWarnings(sources) {
    const { issues, push } = issueList('color');
    for (const s of sources) {
        if (needsColorTransform(s.color))
            push('warning', `${s.id}: ${COLOR_WARNING_MESSAGE}`);
    }
    return issues;
}
 ...
 */
export async function checkMediaFilesExist(m) {
 ...
 */
export function checkKitAssetReferences(m, kitAssets) {
    const { issues, push } = issueList('kit-asset-missing');
    if (!m.kit || kitAssets === undefined)
        return issues;
    const known = new Set(kitAssets.map((a) => a.id));
    const checkRef = (label, ref) => {
    ...
    if (m.composition) {
        ...
        for (const e of m.composition.backgroundTrack ?? [])
            checkRef(`背景切替(t=${e.t.toFixed(1)}s)`, e.ref);
    }
    for (const s of m.timeline.sprites ?? []) {
        if (!known.has(s.assetId))
            push('error', `スプライト ${s.id}: kit素材が見つかりません: ${s.assetId}`);
 ...
 */
export function checkKitDuration(m, actualDurationSeconds, kitProfile) {
    const { issues, push } = issueList('kit-duration');
    if (!m.kit || !kitProfile?.duration_seconds)
        return issues;
    ...
    if (target !== undefined && target > 0) {
        ...
        if (Math.abs(deltaPct) > 10) {
            ...
            push('warning', `尺がキット目標から${sign}${deltaPct.toFixed(1)}%乖離しています(実測${actualDurationSeconds.toFixed(1)}s / 目標${target}s)`);
        }
    }
 ...
 */
export async function staticChecks(m, transcripts, sceneFiles = [], opts = {}) {
...
export function parseSilenceDetect(stderr) {
    ...
    while ((m = re.exec(stderr))) {
        ...
        if (m[1] === 'start') {
            if (pending !== null)
                out.push({ start: pending, end: null, duration: null }); // defensive: two starts in a row, shouldn't happen
            pending = t;
        }
    ...
    if (pending !== null)
        out.push({ start: pending, end: null, duration: null });
    return out;
}
/**
...
function coverageFraction(t0, t1, zones) {
 ...
 */
export async function probeRenderedFile(filePath, opts = {}) {
    ...
    const graph = `[0:v]blackdetect=d=${blackD}:pix_th=0.10[vout];` +
    ...
    for (const b of black) {
        if (coverageFraction(b.start, b.end, zones) >= INTENT_COVERAGE_THRESHOLD)
            continue;
        issues.push({
    ...
    for (const s of silence) {
        ...
        if (coverageFraction(s.start, end, zones) >= INTENT_COVERAGE_THRESHOLD)
            continue;
...
function shotLengthStats(lengths) {
    if (lengths.length === 0)
        return null;
 ...
 */
function silenceRatioFromPeaks(m, peaksBySource, silenceThreshold) {
    let total = 0;
    let silent = 0;
 ...
 * enforcing it.
 */
export function tempoContractLite(m, kitProfile, opts = {}) {
...
function fmtNum(n, digits = 1, suffix = '') {
    if (n === null || n === undefined || !Number.isFinite(n))
        return n === -Infinity ? `-∞${suffix}` : n === Infinity ? `∞${suffix}` : '—';
...
export function buildQcReport(input) {
    ...
    const rows = [
        ...(input.staticReport?.issues ?? []).map((i) => ({ severity: i.severity, category: i.category, message: i.message, tl: i.tlTime })),
        ...(input.probe?.issues ?? []).map((i) => ({ severity: i.severity, category: i.kind, message: i.message, tl: i.t0 })),
    ...
    const rowsHtml = rows.length
        ? rows
            .map((r) => `      <tr class="sev-${r.severity}"${r.tl !== undefined ? ` data-tl="${r.tl}"` : ''}>
