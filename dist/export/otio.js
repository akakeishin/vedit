...
export function toOtio(m) {
    ...
    const clipObjs = (kind) => segments(m).map((seg, i) => {
        ...
        metadata: {
            vedit: {
                ...
                ...(hasReframe(m)
                    ? {
                        output: m.output,
                        reframeNote: 'crop/output are recorded under each clip\'s metadata.vedit.crop, but OTIO has no standard transform field — most importers (including Resolve) will not visually apply the reframe on import.',
                    }
                    : {}),
                ...(hasOverlayTransform(m)
                    ? {
                        overlayTransformNote: 'overlay rect/opacity/fade are recorded under each overlay clip\'s metadata.vedit, but OTIO has no standard transform field — most importers (including Resolve) will not visually apply the placement/opacity/fade on import.',
                    }
                    : {}),
            },
        },
...
export async function writeOtio(m, outPath) {
