 ...
 */
export function wrapSrtLine(text, maxChars) {
...
function srtFromCues(cues, maxChars) {
    return cues
        .map((c, i) => `${i + 1}\n${srtTime(c.tlStart)} --> ${srtTime(c.tlEnd)}\n${wrapSrtLine(c.text, maxChars)}\n`)
        .join('\n');
}
export function toSrt(m, transcripts) {
 ...
 */
export async function writeSrt(m, transcripts, outPath) {
    const { cues, excluded } = captionCuesWithExclusions(m, transcripts);
    if (excluded.length > 0)
        console.error(`警告: ${formatCaptionExclusionWarning(excluded)}`);
