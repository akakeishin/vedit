...
function alphaHexFromOpacity(opacity) {
...
function colorTag(n, hex, opacity = 1) {
...
function clampFadeMs(preferredMs, durationSeconds) {
 ...
 */
function chapterCardLines(item, spec, output, accentHex) {
    ...
    lines.push(`Dialogue: 0,${start},${end},${MOTION_STYLE_NAME},,0,0,0,,` +
 ...
 */
function lowerThirdLines(item, spec, output, accentHex) {
    const { width: W, height: H } = output;
    const start = assTime(item.tlStart);
    const end = assTime(item.tlStart + item.duration);
    ...
    const lineH = (fs) => Math.round(fs * 1.3);
 ...
 */
function calloutLines(item, spec, output, accentHex) {
    const { width: W, height: H } = output;
    const start = assTime(item.tlStart);
    const end = assTime(item.tlStart + item.duration);
 ...
 */
function ctaLines(item, spec, output, accentHex) {
    const { width: W, height: H } = output;
    const start = assTime(item.tlStart);
    const end = assTime(item.tlStart + item.duration);
    const fadeMs = clampFadeMs(350, item.duration);
 ...
 */
export function motionAssLines(items, output, defaultAccentHex) {
    const lines = [];
    ...
    for (const { item, spec } of items) {
        ...
        if (!BURNABLE_TYPES.has(spec.type))
            continue; // unrecognized/future type — not one of the 4 presets, skip silently
        const accentHex = motionAccentHex(spec, defaultAccentHex);
        if (spec.type === 'chapter-card')
            lines.push(...chapterCardLines(item, spec, output, accentHex));
        else if (spec.type === 'lower-third')
            lines.push(...lowerThirdLines(item, spec, output, accentHex));
        else if (spec.type === 'callout')
            lines.push(...calloutLines(item, spec, output, accentHex));
        else if (spec.type === 'cta')
            lines.push(...ctaLines(item, spec, output, accentHex));
    }
 ...
 */
export function buildMotionAss(m, motionSpecs, kit, output) {
