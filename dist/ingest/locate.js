...
function escapeMdfindLiteral(s) {
 ...
 */
export function mdfindByName(name, execFileImpl = execFile) {
 ...
 */
export async function fingerprintFile(filePath, expectedSize) {
    let stat;
    try {
    ...
    try {
        ...
        if (headLen > 0)
            await fh.read(headBuf, 0, headLen, headStart);
 ...
 */
export async function locateMedia(name, target, deps = {}) {
    ...
    for (const candidatePath of candidates) {
        ...
        if (fp && fingerprintsMatch(fp, target))
