...
export function ffmpegBin() {
    ...
    for (const c of FFMPEG_CANDIDATES) {
        try {
            ...
            if (!resolvedFfmpeg)
                resolvedFfmpeg = c; // usable fallback without drawtext
        }
        catch { /* try next */ }
    }
    ...
    return new Promise((resolve, reject) => {
        execFile(cmd, args, { maxBuffer: opts.maxBuffer ?? 64 * 1024 * 1024, signal: opts.signal }, (err, stdout, stderr) => {
            if (err && opts.signal?.aborted) {
                ...
                reject(Object.assign(new Error(`${cmd} failed: ${errExcerpt(stderr) || err.message}`), { code: err.code, signal: err.signal, stderr }));
            }
            else
                resolve(stdout);
        });
    ...
    return new Promise((resolve, reject) => {
        execFile(cmd, args, { maxBuffer: opts.maxBuffer ?? 64 * 1024 * 1024, signal: opts.signal }, (err, stdout, stderr) => {
            if (err && opts.signal?.aborted) {
                ...
                reject(new Error(`${cmd} failed: ${errExcerpt(stderr) || err.message}`));
            else
...
export function runBinary(cmd, args, opts = {}) {
    if (cmd === 'ffmpeg')
        cmd = ffmpegBin();
    return new Promise((resolve, reject) => {
        execFile(cmd, args, { maxBuffer: opts.maxBuffer ?? 1024 * 1024 * 1024, encoding: 'buffer', signal: opts.signal }, (err, stdout, stderr) => {
            if (err && opts.signal?.aborted) {
                const aborted = new Error('operation cancelled');
                aborted.name = 'AbortError';
                reject(aborted);
                ...
                reject(new Error(`${cmd} failed: ${errExcerpt(stderr.toString())}`));
            else
                resolve(stdout);
        });
    });
