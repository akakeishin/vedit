 ...
 */
export function summarizeFirstDraftForCli(draft, detected, exampleLimit = 5) {
    ...
    const questionCount = Number.isFinite(draft.questionCount)
        ...
        : questions.length;
    const reasonCounts = Object.fromEntries([...questions.reduce((counts, item) => {
 ...
 */
export function planAutonomousCandidateBatch(m, candidates) {
    const autoApply = [];
    const needsDecision = [];
    const excluded = [];
    const fragmentsAbsorbed = [];
    let preview = m;
    const ordered = [...candidates].sort((a, b) => a.sourceId.localeCompare(b.sourceId) || a.t0 - b.t0 || a.t1 - b.t1 || a.id.localeCompare(b.id));
    for (const c of ordered) {
        const decision = baseDecision(m, c);
        if (decision.reasonCode === 'already-decided' ||
        ...
        const absorbed = next.fragmentsAbsorbed ?? [];
