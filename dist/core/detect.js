...
export function detectSilences(t, minGap = 0.7, pad = 0.12) {
    const out = [];
    const w = t.words;
 ...
 */
export function detectFillers(t, minFlankGap = 0.15) {
    ...
    for (let i = 0; i < w.length; i++) {
        ...
        if (before < requiredGap && after < requiredGap)
            continue; // mid-sentence, keep
 ...
 */
function clampToWordBoundaries(t0, t1, words) {
    let a = t0;
    let b = t1;
    for (const w of words) {
        if (w.t1 <= w.t0)
            continue; // zero-width word, ignore
        if (w.t1 <= a || w.t0 >= b)
            continue; // no overlap
        if (w.t0 <= a && w.t1 >= b)
            return null; // word covers the whole window
        if (w.t0 <= a)
            a = w.t1; // word overlaps the left edge
        else if (w.t1 >= b)
            b = w.t0; // word overlaps the right edge
        else {
 ...
 */
export function adaptiveThreshold(peaks) {
 ...
 */
export function timestampsArePacked(words, ratio = 0.8) {
    let zero = 0;
    let total = 0;
    for (let i = 1; i < words.length; i++) {
        total++;
        if (words[i].t0 - words[i - 1].t1 <= 0.001)
            zero++;
    }
    return total >= 5 && zero / total >= ratio;
}
export function detectSilencesFromPeaks(peaks, opts) {
    ...
    const silent = new Array(peaks.peaks.length);
    ...
    while (i < silent.length) {
        if (!silent[i]) {
            let j = i;
            while (j < silent.length && !silent[j])
                j++;
            if (j - i < minSpeechSamples)
                for (let k = i; k < j; k++)
                    silent[k] = true;
            i = j;
        }
