 ...
 */
export function packTranscript(m, t, candidates = [], idEvery = 8) {
    const words = keptWords(m, t.sourceId, t.words);
    if (words.length === 0)
        return '(no speech on the timeline for this source)';
    const candByWord = new Map();
