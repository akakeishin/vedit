...
export function captionCueKey(sourceId, wordId) {
...
const ANNOTATION_BRACKET_PAIRS = [
    ['(', ')'],
    ['[', ']'],
    ['【', '】'],
];
...
function stripOuterAnnotationBrackets(s) {
...
export function isNonSpeechAnnotation(text) {
    ...
    if (trimmed.replace(NOTE_SYMBOLS_RE, '').trim() === '')
        return true;
    const inner = stripOuterAnnotationBrackets(trimmed);
    if (inner === null)
        return false; // no full-cue bracket wrap -> never excluded
    const normalized = inner
        ...
        .toLowerCase();
    if (!normalized)
        return true; // e.g. "[...]" / "[♪]" — bracketed but empty of words
 ...
 */
export function formatCaptionExclusionWarning(excluded) {
 ...
 */
function enforceMinDisplay(cues, total, maxCps) {
    let i = 0;
    while (i < cues.length) {
        const c = cues[i];
 ...
 */
export function captionCues(m, transcripts) {
 ...
 * are shown directly.
 */
export function captionCuesWithExclusions(m, transcripts) {
    if (!m.captions.enabled)
        return { cues: [], excluded: [] };
    const cues = [];
    ...
    for (const t of transcripts) {
        ...
        for (const seg of segsForSource) {
            ...
            for (const w of t.words) {
                ...
                if (mid < seg.srcStart || mid >= seg.srcStart + segDur)
                    continue;
                ...
                if (prev && (tl0 - prev.tl1 > 0.6 || lineLen + w.text.length > m.captions.maxChars))
                    flush();
    ...
    for (let i = 0; i < cues.length - 1; i++) {
        if (cues[i].tlEnd > cues[i + 1].tlStart)
            cues[i].tlEnd = cues[i + 1].tlStart;
    }
