 ...
 */
export function parseSceneChangeTimes(showinfoOutput) {
    const times = new Set();
...
export async function detectSceneChangeTimes(mediaPath, sensitivity = 0.3, opts = {}) {
 ...
 */
export function mergeShortRanges(ranges, minLen) {
 ...
 */
export function buildSceneRanges(duration, boundaryTimes, opts = {}) {
 ...
 * different range.
 */
export function assignSceneIds(ranges, existing = [], tolerance = 0.5) {
    let maxNum = 0;
...
export function computeHasSpeech(t0, t1, keptTranscriptWords) {
    return keptTranscriptWords.some((w) => w.t1 > t0 && w.t0 < t1);
}
/** Mean waveform peak over [t0, t1), as a motion proxy. */
export function computeEnergy(peaks, t0, t1) {
 ...
 */
export async function sceneThumbPath(project, sourceId, sceneId) {
 ...
 */
export async function detectScenesForSource(project, m, sourceId, opts = {}) {
        ...
        try {
            ...
            try {
                ...
                for (const item of replaced.reverse()) {
                    if (item.hadPrevious)
                        await fs.rename(item.backup, item.abs).catch(() => { });
                    else
                        await fs.rm(item.abs, { force: true }).catch(() => { });
                }
                throw error;
            }
        }
 ...
 */
export function packScenes(file, review = {}) {
    if (file.scenes.length === 0)
        return '(no scenes detected; run `vedit scenes detect`)';
