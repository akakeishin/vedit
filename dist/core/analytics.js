...
function classifyHeaderCell(norm) {
    if (POSITION_HEADER_ALIASES.includes(norm))
        return 'position';
    if (RETENTION_HEADER_ALIASES.includes(norm))
        return 'retention';
    ...
    if (/位置|position/.test(norm))
        return 'position';
    if (/維持率|retention/.test(norm))
        return 'retention';
    return null;
}
 ...
 */
export function parseRetentionCsv(text) {
    ...
    let posIdx = -1;
    let retIdx = -1;
    let firstNonBlankCells = null;
    for (let i = 0; i < maxScan; i++) {
        const raw = lines[i];
        if (!raw || !raw.trim())
            continue;
        const cells = splitCsvLine(raw);
        if (firstNonBlankCells === null)
            firstNonBlankCells = cells;
        let pIdx = -1;
        let rIdx = -1;
        for (let c = 0; c < cells.length; c++) {
            const kind = classifyHeaderCell(normalizeHeaderCell(cells[c]));
            if (kind === 'position' && pIdx === -1)
                pIdx = c;
            else if (kind === 'retention' && rIdx === -1)
                rIdx = c;
        }
    ...
    if (headerLineIndex === -1) {
        ...
        throw new Error(`parseRetentionCsv: unrecognized retention CSV format` +
    ...
    for (let i = headerLineIndex + 1; i < lines.length; i++) {
        ...
        if (cells.length <= Math.max(posIdx, retIdx))
            continue;
...
function findWordAt(transcripts, sourceId, srcTime) {
...
function findSceneAt(sceneFiles, sourceId, srcTime) {
    const sf = sceneFiles?.find((s) => s.sourceId === sourceId);
    if (!sf)
        return null;
    return sf.scenes.find((sc) => srcTime >= sc.t0 && srcTime < sc.t1) ?? null;
}
function findChapterAt(chapters, tl) {
    if (!chapters || chapters.length === 0)
        return null;
    let best = null;
    for (const c of chapters) {
        if (c.tlTime <= tl && (!best || c.tlTime > best.tlTime))
            best = c;
    }
    return best;
}
 ...
 */
export function mapRetentionToTimeline(points, renderDurationSeconds, m, transcripts, sceneFiles, chapters) {
 ...
 */
export function findDipsAndSpikes(points, opts = {}) {
    if (points.length === 0)
        return { introDropPct: 0, dips: [], spikes: [] };
    ...
    for (let i = 0; i < mainEntries.length; i++) {
        if (i - windowSize < 0 || i + windowSize >= mainEntries.length)
            continue; // insufficient symmetric context to judge trend
        const neighbors = [];
        ...
        if (delta <= -dipThresholdPct)
            dips.push(point);
        else if (delta >= spikeThresholdPct)
            spikes.push(point);
    }
...
function nearbyQuote(transcripts, sourceId, srcTime, windowSec = 3) {
    const t = transcripts.find((tr) => tr.sourceId === sourceId);
    if (!t)
        return null;
    const near = t.words
 ...
 * module guesses at.
 */
export function buildRetrospective(points, renderDurationSeconds, m, transcripts, sceneFiles, chapters, opts) {
