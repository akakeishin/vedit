 ...
 */
export function timelineDuration(m) {
    if (m.composition)
        return m.composition.duration;
...
export function sourceRangeToTimeline(m, sourceId, t0, t1) {
    let start = null;
    let end = null;
 ...
 */
export function removeSourceRange(m, sourceId, t0, t1) {
    ...
    for (const c of m.timeline.video) {
        ...
        const leftCandidate = a > c.srcIn ? { ...c, srcOut: Math.min(a, c.srcOut) } : null;
        ...
        if (leftCandidate && leftDur > 1e-6 && !keepLeft)
            fragmentsAbsorbed.push({ side: 'before', seconds: leftDur });
        const rightCandidate = b < c.srcOut ? { ...c, id: keepLeft ? freshId('c') : c.id, srcIn: Math.max(b, c.srcIn) } : null;
        ...
        if (rightCandidate && rightDur > 1e-6 && !keepRight)
            fragmentsAbsorbed.push({ side: 'after', seconds: rightDur });
        if (keepLeft)
            next.push(leftCandidate);
        if (keepRight)
            next.push(rightCandidate);
    }
    const result = { ...m, timeline: { ...m.timeline, video: next } };
...
export function wordRange(words, ids) {
    const set = new Set(ids);
    const hit = words.filter((w) => set.has(w.id));
    if (hit.length === 0)
        throw new Error(`no words matched ids: ${ids.slice(0, 5).join(',')}...`);
 ...
 */
export function padWordRange(words, ids, range, pad) {
    ...
    for (const w of words) {
        if (idSet.has(w.id))
            continue;
        if (w.t1 <= range.t0 && w.t1 > t0)
            t0 = w.t1;
        if (w.t0 >= range.t1 && w.t0 < t1)
            t1 = w.t0;
    }
    t0 = Math.max(0, t0);
...
export function expandWordIds(spec, words) {
    ...
    for (const s of spec) {
        ...
        if (!m) {
            if (!order.has(s))
                throw new Error(`unknown word id: ${s}`);
            out.push(s);
            continue;
        }
        ...
        if (i0 === undefined || i1 === undefined)
            throw new Error(`unknown word id in range: ${s}`);
...
export function trimClip(m, clipId, edge, frames) {
    if (edge !== 'in' && edge !== 'out') {
        throw new Error(`invalid edge: ${JSON.stringify(edge)} (must be "in" or "out")`);
    }
    if (!Number.isInteger(frames))
        throw new Error(`invalid frames: ${JSON.stringify(frames)} (must be a finite integer)`);
    ...
    if (!m.timeline.video.some((c) => c.id === clipId))
...
export function keptWords(m, sourceId, words) {
...
export function sourceTimeToTimeline(m, sourceId, t) {
    ...
    for (const s of segments(m)) {
        ...
        if (t >= s.srcStart && t < s.srcStart + d)
            return s.tlStart + (t - s.srcStart);
    }
    return null;
}
/** Map a timeline time to (sourceId, source time), or null if out of range — the inverse of sourceTimeToTimeline. */
export function timelineTimeToSource(m, tl) {
    for (const s of segments(m)) {
        if (tl >= s.tlStart && tl < s.tlEnd)
            return { sourceId: s.sourceId, srcTime: s.srcStart + (tl - s.tlStart) };
    }
    return null;
}
// ---- clip selection / reorder (timeline vs. source pool) ----
/** Add a clip to the timeline referencing an already-ingested source. */
export function addClip(m, sourceId, opts = {}) {
    ...
    if (srcIn < 0)
        throw new Error(`clip-add: in (${srcIn}) must be >= 0`);
    if (srcOut <= srcIn)
        throw new Error(`clip-add: out (${srcOut}) must be greater than in (${srcIn})`);
    ...
    const clip = { id, sourceId, srcIn, srcOut };
...
export function removeClip(m, clipId) {
    const video = m.timeline.video.filter((c) => c.id !== clipId);
    if (video.length === m.timeline.video.length)
        throw new Error(`unknown clip: ${clipId}`);
    return { ...m, timeline: { ...m.timeline, video } };
}
/** Reorder a clip to just before `beforeClipId`, or to the end when 'end'. */
export function moveClip(m, clipId, beforeClipId) {
    ...
    if (beforeClipId !== 'end') {
        ...
        if (at < 0)
            throw new Error(`unknown clip: ${beforeClipId}`);
    }
 ...
 */
export function splitClip(m, clipId, tlTime) {
    if (!Number.isFinite(tlTime))
        throw new Error(`split: invalid tlTime (${tlTime})`);
    const idx = m.timeline.video.findIndex((c) => c.id === clipId);
    if (idx < 0)
        throw new Error(`unknown clip: ${clipId}`);
    const seg = segments(m).find((s) => s.clipId === clipId);
    if (!seg)
        throw new Error(`clip has no timeline extent: ${clipId}`);
    const fps = m.fps;
    ...
    if (leftDur < minFrame - 1e-9 || rightDur < minFrame - 1e-9) {
        throw new Error(`split: ${tlTime}s is too close to clip ${clipId}'s edge (need at least one frame of ${minFrame.toFixed(4)}s on both sides; clip spans ${seg.tlStart.toFixed(3)}..${seg.tlEnd.toFixed(3)})`);
    }
    const clip = m.timeline.video[idx];
    const srcSplit = seg.srcStart + (t - seg.tlStart);
    const left = { ...clip, srcOut: srcSplit };
    const right = { ...clip, id: freshId('c'), srcIn: srcSplit };
    const video = [...m.timeline.video];
 ...
 */
export function duplicateClip(m, clipId) {
    const idx = m.timeline.video.findIndex((c) => c.id === clipId);
    if (idx < 0)
        throw new Error(`unknown clip: ${clipId}`);
    const clip = m.timeline.video[idx];
    const dup = { ...clip, id: freshId('c') };
    const video = [...m.timeline.video];
...
export function parseReframeSpec(spec) {
    ...
    if (literal)
        return { width: normalizeDim(Number(literal[1])), height: normalizeDim(Number(literal[2])) };
    const ratio = spec.match(/^(\d+):(\d+)$/);
    if (!ratio)
        throw new Error(`invalid reframe spec: ${spec} (use 9:16, 1:1, 16:9, or WxH)`);
    const w = Number(ratio[1]);
    const h = Number(ratio[2]);
    if (w <= 0 || h <= 0)
        throw new Error(`invalid reframe spec: ${spec} (ratio parts must be positive)`);
...
export function parseFocus(focus) {
    ...
    if (Number.isNaN(n))
        throw new Error(`invalid focus: ${focus} (use left/center/right or 0..1)`);
 ...
 */
export function cropWindow(srcW, srcH, outW, outH) {
    const srcAspect = srcW / srcH;
    const outAspect = outW / outH;
    if (Math.abs(srcAspect - outAspect) < 1e-6)
        return { width: srcW, height: srcH, axis: 'none' };
 ...
 */
export function cropOffset(sourceDim, windowDim, pos) {
...
export function cropGeometry(srcW, srcH, outW, outH, crop) {
...
export function applyReframe(m, output, focus) {
    return {
        ...m,
        output,
...
function assertNonNegative(v, label, name) {
    if (!Number.isFinite(v) || v < 0) {
 ...
 */
export function addMusic(m, path, opts) {
    ...
    if (music.some((x) => x.id === id))
        throw new Error(`music-add: id already exists: ${id}`);
    const item = { id, path, tlStart, duration, srcIn, gain, fadeIn, fadeOut, duck, ...(opts.role ? { role: opts.role } : {}) };
    return { ...m, timeline: { ...m.timeline, music: [...music, item] } };
}
/** Patch an existing music item's placement/mix fields (never its path). */
export function updateMusic(m, id, patch) {
    const music = m.timeline.music ?? [];
    const idx = music.findIndex((x) => x.id === id);
    if (idx < 0)
        throw new Error(`unknown music item: ${id}`);
    const cur = music[idx];
    const next = { ...cur };
...
export function setAudioRepair(m, patch) {
    ...
    const next = { preset: patch.preset };
    if (patch.deess !== undefined)
        next.deess = Boolean(patch.deess);
 ...
 */
export function needsColorTransform(color) {
    if (!color)
        return false;
    const transfer = color.transfer;
    const KNOWN_SDR_TRANSFERS = new Set(['bt709', 'srgb']);
    if (transfer && transfer !== 'unknown' && !KNOWN_SDR_TRANSFERS.has(transfer))
        return true;
    if ((!transfer || transfer === 'unknown') && color.primaries === 'bt2020')
        return true;
    return false;
}
 ...
 */
export function setColorTransform(m, sourceId, patch) {
    if (!m.sources.some((s) => s.id === sourceId))
        throw new Error(`unknown source: ${sourceId}`);
    ...
    const colorTransform = { type: patch.type };
    if (patch.type === 'lut')
        colorTransform.lut = patch.lut;
...
function assertColorRange(v, name, min, max) {
 ...
 */
export function setColorAdjust(m, sourceId, patch) {
    if (!m.sources.some((s) => s.id === sourceId))
        throw new Error(`unknown source: ${sourceId}`);
    if (patch.exposure !== undefined)
        assertColorRange(patch.exposure, 'exposure', -2, 2);
    if (patch.wb !== undefined)
        assertColorRange(patch.wb, 'wb', -100, 100);
    if (patch.sat !== undefined)
        assertColorRange(patch.sat, 'sat', 0, 2);
    const colorAdjust = { ...(m.colorAdjust ?? {}) };
    const cur = { ...(colorAdjust[sourceId] ?? {}) };
    if (patch.exposure !== undefined)
        cur.exposure = patch.exposure;
    if (patch.wb !== undefined)
        cur.wb = patch.wb;
    if (patch.sat !== undefined)
        cur.sat = patch.sat;
    if (Object.keys(cur).length === 0)
        delete colorAdjust[sourceId];
    else
        colorAdjust[sourceId] = cur;
    return { ...m, colorAdjust };
}
/** Adjust one clip's crop position without touching the others. */
export function setClipCrop(m, clipId, patch) {
    if (!m.timeline.video.some((c) => c.id === clipId))
        throw new Error(`unknown clip: ${clipId}`);
    for (const [axis, v] of [['x', patch.x], ['y', patch.y]]) {
 ...
 * "プレビュー未反映" warning.
 */
export function setClipAudio(m, clipId, patch) {
    if (!m.timeline.video.some((c) => c.id === clipId))
        throw new Error(`unknown clip: ${clipId}`);
 ...
 */
export function setSceneReview(m, sourceId, sceneId, review) {
    if (!m.sources.some((s) => s.id === sourceId))
        throw new Error(`unknown source: ${sourceId}`);
    ...
    if (Object.keys(forSource).length === 0)
        delete culling[sourceId];
    else
        culling[sourceId] = forSource;
    return { ...m, culling };
}
/** Per-source and overall keep/reject/unreviewed tallies, for status reporting. */
export function cullingStats(m, sceneFiles) {
 ...
 */
export function buildSelectsTimeline(m, sceneFiles, opts) {
    ...
    const unionBySource = new Map();
    if (!raw) {
        for (const c of m.timeline.video) {
            if (c.srcOut - c.srcIn <= EPS)
                continue;
    ...
    Object.defineProperty(result, 'summary', {
        value: { keepScenes, clips: out.length, preservedScenes, newScenes, raw },
        enumerable: false,
        configurable: true,
    });
    return result;
 ...
 */
export function resolveOverlays(m) {
...
export function overlayLayerOf(o) {
    return o.layer ?? 1;
}
/**
 ...
 */
export function resolvedActiveOverlays(m) {
    const out = [];
...
export function orphanedOverlays(m) {
...
function resolvedOverlayRange(m, o) {
 ...
 */
function assertNoOverlayOverlap(m, candidate, excludeId) {
    ...
    for (const o of m.timeline.overlays ?? []) {
        if (o.id === excludeId)
            continue;
        if (overlayLayerOf(o) !== candidateLayer)
            continue;
        const other = resolvedOverlayRange(m, o);
        if (!other)
            continue;
        if (range.tlStart < other.tlEnd && other.tlStart < range.tlEnd) {
            throw new Error(`broll: overlaps existing overlay ${o.id} on layer ${candidateLayer} (${other.tlStart.toFixed(2)}-${other.tlEnd.toFixed(2)}s); a single layer allows no overlap (use a different --layer to stack)`);
        }
    }
}
...
function assertOverlayLayer(layer, label) {
...
function assertOverlayRect(rect, label) {
...
function assertOverlayFade(fade, label) {
 ...
 * "layer 1 の別名".
 */
export function addOverlay(m, sourceId, opts) {
    const src = m.sources.find((s) => s.id === sourceId);
    if (!src)
        throw new Error(`broll-add: unknown B-roll source: ${sourceId}`);
    ...
    if (srcIn < 0)
        throw new Error(`broll-add: in (${srcIn}) must be >= 0`);
    if (srcOut <= srcIn)
        throw new Error(`broll-add: out (${srcOut}) must be greater than in (${srcIn})`);
    if (srcOut > src.duration)
        throw new Error(`broll-add: out (${srcOut}) exceeds source duration (${src.duration})`);
    ...
    if (opts.gainDb !== undefined)
        assertGain(opts.gainDb, 'broll-add');
    if (opts.layer !== undefined)
        assertOverlayLayer(opts.layer, 'broll-add');
    if (opts.rect !== undefined)
        assertOverlayRect(opts.rect, 'broll-add');
    if (opts.opacity !== undefined)
        assertUnit(opts.opacity, 'broll-add', 'opacity');
    if (opts.fade !== undefined)
        assertOverlayFade(opts.fade, 'broll-add');
    const overlays = m.timeline.overlays ?? [];
    const id = opts.id ?? freshId('ov');
    if (overlays.some((o) => o.id === id))
        throw new Error(`broll-add: overlay id already exists: ${id}`);
    const item = {
        id,
        sourceId,
        srcIn,
 ...
 * outright.
 */
export function updateOverlay(m, id, patch) {
    const overlays = m.timeline.overlays ?? [];
    ...
    if (patch.srcIn !== undefined || patch.srcOut !== undefined) {
        ...
        if (srcIn < 0)
            throw new Error(`broll-update: in (${srcIn}) must be >= 0`);
        if (srcOut <= srcIn)
            throw new Error(`broll-update: out (${srcOut}) must be greater than in (${srcIn})`);
        if (srcOut > src.duration)
            throw new Error(`broll-update: out (${srcOut}) exceeds source duration (${src.duration})`);
        next.srcIn = srcIn;
        next.srcOut = srcOut;
    }
...
export function removeOverlay(m, id) {
    const overlays = m.timeline.overlays ?? [];
 ...
 *    warn about there.
 */
export function overlayGeometryWarnings(m) {
    ...
    for (const r of resolvedActiveOverlays(m)) {
        ...
        if (r.tlEnd > total + 1e-6) {
            warnings.push(`オーバーレイ ${ov.id}: タイムライン終端(${total.toFixed(2)}s)を超えて${r.tlEnd.toFixed(2)}sまで配置されています(はみ出し分は書き出しに反映されません)`);
        }
        if (!ov.rect) {
            ...
            if (src && src.width > 0 && src.height > 0) {
                ...
                if (srcLandscape !== outLandscape) {
                    warnings.push(`オーバーレイ ${ov.id}: 素材(${src.width}x${src.height})の向きが出力(${output.width}x${output.height})と異なり --rect 未指定のため全面表示されます(引き伸ばし/レターボックスに注意)`);
                }
            }
        }
    }
...
export function resolveSprites(m) {
 ...
 */
export function resolvedActiveSprites(m) {
    const out = [];
...
export function orphanedSprites(m) {
...
function assertUnit(v, label, name) {
 ...
 */
function isKnownAnchorSource(m, sourceId) {
    if (m.composition && sourceId === COMP_SOURCE_ID)
        return true;
    return m.sources.some((s) => s.id === sourceId);
}
const SPRITE_MOTION_NAMES = new Set(['slide-left', 'slide-right', 'hop-in', 'pop', 'fade']);
const SPRITE_LOOP_NAMES = new Set(['sway', 'bob', 'hop', 'breathe', 'none']);
/**
 ...
 */
function assertSpriteMotion(motion, label) {
    if (motion === undefined)
        return undefined;
    ...
    if (motion.emoteAt !== undefined) {
        if (!Array.isArray(motion.emoteAt))
            throw new Error(`${label}: motion.emoteAt must be an array`);
...
export function addSprite(m, assetId, opts) {
    if (typeof assetId !== 'string' || !assetId)
        throw new Error('sprite-add: assetId is required');
    ...
    if (sprites.some((s) => s.id === id))
        throw new Error(`sprite-add: sprite id already exists: ${id}`);
...
export function updateSprite(m, id, patch) {
    const sprites = m.timeline.sprites ?? [];
...
export function removeSprite(m, id) {
    const sprites = m.timeline.sprites ?? [];
    const next = sprites.filter((s) => s.id !== id);
    if (next.length === sprites.length)
        throw new Error(`unknown sprite: ${id}`);
 ...
 */
export function spriteGeometry(asset, position, scale, outputWH, opts = {}) {
...
const NO_TERMS = { dx: '', dy: '', fade: null };
/** Entrance-transition terms, active during [tlStart, tlStart+D). */
function enterTerms(name, tlStart, geo) {
    if (!name)
        return NO_TERMS;
...
function exitTerms(name, tlEnd, geo) {
    if (!name)
        return NO_TERMS;
    const D = SPRITE_TRANSITION_SECONDS;
    const start = tlEnd - D;
...
function loopTerms(loop, tlStart) {
    if (!loop || loop === 'none' || loop === 'breathe')
        return { dx: '', dy: '' };
    const lt = `(t-${fexpr(tlStart)})`;
    if (loop === 'sway')
        return { dx: `+${SPRITE_LOOP_SWAY_PX}*sin(2*PI*${lt}/3)`, dy: '' };
    if (loop === 'bob')
        return { dx: '', dy: `+${SPRITE_LOOP_BOB_PX}*sin(2*PI*${lt}/2.4)` };
 ...
 */
export function spriteMotionPlan(motion, geo, tlStart, tlEnd) {
    ...
    const fadeClauses = [enter.fade, exit.fade].filter((c) => Boolean(c));
 ...
 * sprite-local window.
 */
export function emoteWindows(emoteAt, duration) {
...
function assertBackgroundRef(ref, label) {
    if (!ref || typeof ref !== 'object' || Array.isArray(ref))
        throw new Error(`${label}: background ref is required`);
    if (ref.type === 'color') {
        if (typeof ref.hex !== 'string' || !COMP_HEX_COLOR_RE.test(ref.hex)) {
            throw new Error(`${label}: invalid hex color: ${JSON.stringify(ref.hex)}`);
        }
    }
    else if (ref.type === 'asset') {
        if (typeof ref.assetId !== 'string' || !ref.assetId)
            throw new Error(`${label}: assetId is required`);
    }
        ...
        throw new Error(`${label}: background type must be "color", "asset", or "video" (got ${JSON.stringify(ref?.type)})`);
    }
}
/**
 ...
 */
export function setComposition(m, opts) {
    ...
    const composition = { ...prev, duration: opts.duration, background };
    if (composition.backgroundTrack) {
        const inRange = composition.backgroundTrack.filter((e) => e.t < opts.duration);
        if (inRange.length)
            composition.backgroundTrack = inRange;
        else
            delete composition.backgroundTrack;
    }
    return {
        ...m,
 ...
 */
export function setBackgroundAt(m, t, ref) {
    if (!m.composition)
        throw new Error('bg-set: project has no composition (run `vedit compose` first)');
    if (!Number.isFinite(t) || t < 0)
        throw new Error(`bg-set: at (${t}) must be a finite number >= 0`);
    ...
    if (idx >= 0)
        track[idx] = { t: tSnapped, ref };
    else
        track.push({ t: tSnapped, ref });
...
export function removeBackgroundAt(m, t) {
    if (!m.composition)
        throw new Error('bg-remove: project has no composition');
    const fps = m.fps;
    if (t < 0.5 / fps)
        throw new Error('bg-remove: t=0 is the base background — replace it with `vedit bg-set --at 0 --to ...` instead of removing it');
    const track = m.composition.backgroundTrack ?? [];
    const next = track.filter((e) => Math.abs(e.t - t) >= 0.5 / fps);
    if (next.length === track.length)
        throw new Error(`bg-remove: no background cut at t=${t}`);
...
export function resolvedBackgroundAt(m, t) {
    if (!m.composition)
        throw new Error('resolvedBackgroundAt: project has no composition');
    let active = m.composition.background;
    for (const e of m.composition.backgroundTrack ?? []) {
        if (e.t <= t)
            active = e.ref;
        else
            break; // backgroundTrack is kept sorted ascending by setBackgroundAt
    }
    return active;
}
...
export function backgroundIntervals(m) {
    if (!m.composition)
        return [];
 ...
 */
export function shiftComposition(m, from, by, opts) {
    if (!m.composition) {
        throw new Error('shift: コンポジション専用の操作です。実写プロジェクトはカット編集が自動リップルします — remove-range / remove-words / clip-move 等を使ってください');
    }
    if (!Number.isFinite(from) || from < 0)
        throw new Error(`shift: from (${from}) must be a finite number >= 0`);
    if (!Number.isFinite(by) || by === 0)
        throw new Error(`shift: by (${by}) must be a finite, non-zero number of seconds`);
    const EPS = 1e-6;
    ...
    const check = (label, id, tNew, moved, tOld) => {
        if (moved && tNew < -EPS) {
    ...
    if (m.timeline.sprites)
        timeline.sprites = sprites;
    if (m.timeline.dialogue)
...
export function addDialogue(m, text, opts) {
    if (typeof text !== 'string' || !text.trim())
        throw new Error('dialogue-add: text is required');
    ...
    if (dialogue.some((d) => d.id === id))
        throw new Error(`dialogue-add: id already exists: ${id}`);
    const item = {
        id,
        text,
...
export function updateDialogue(m, id, patch) {
    const dialogue = m.timeline.dialogue ?? [];
    const idx = dialogue.findIndex((d) => d.id === id);
    if (idx < 0)
        throw new Error(`unknown dialogue item: ${id}`);
    const cur = dialogue[idx];
    const next = { ...cur };
    if (patch.text !== undefined) {
        if (typeof patch.text !== 'string' || !patch.text.trim())
            throw new Error('dialogue-update: text must be a non-empty string');
 ...
 */
export function dialogueOverlapWithoutPosRisk(m, candidate) {
    if (candidate.pos)
        return false;
    const cEnd = candidate.tlStart + candidate.duration;
    return (m.timeline.dialogue ?? []).some((d) => {
        if (candidate.excludeId && d.id === candidate.excludeId)
            return false;
        if (d.pos)
            return false;
...
export function removeDialogue(m, id) {
    const dialogue = m.timeline.dialogue ?? [];
    const next = dialogue.filter((d) => d.id !== id);
    if (next.length === dialogue.length)
        throw new Error(`unknown dialogue item: ${id}`);
...
export function addIntentZone(m, sourceId, t0, t1, opts) {
    if (!m.sources.some((s) => s.id === sourceId))
        throw new Error(`intent-add: unknown source: ${sourceId}`);
    if (!Number.isFinite(t0) || !Number.isFinite(t1)) {
        throw new Error(`intent-add: t0/t1 must be finite numbers (got t0=${t0}, t1=${t1})`);
    }
    if (t0 < 0)
        throw new Error(`intent-add: t0 (${t0}) must be >= 0`);
    if (t1 <= t0)
        throw new Error(`intent-add: t1 (${t1}) must be greater than t0 (${t0})`);
    ...
    if (zones.some((z) => z.id === id))
        throw new Error(`intent-add: id already exists: ${id}`);
    const zone = { id, sourceId, t0, t1, label: opts.label, kind };
    return { ...m, intentZones: [...zones, zone] };
}
/** Remove a protection zone. */
export function removeIntentZone(m, id) {
    const zones = m.intentZones ?? [];
    const next = zones.filter((z) => z.id !== id);
    if (next.length === zones.length)
        throw new Error(`unknown intent zone: ${id}`);
    return { ...m, intentZones: next };
}
/** Every intent zone for one source, source-domain (unfiltered by kind). */
export function intentZonesForSource(m, sourceId) {
...
export function overlappingIntentZones(zones, t0, t1) {
 ...
 */
export function quietZonesOverlappingTimelineRange(m, tlStart, tlEnd) {
    const zones = m.intentZones ?? [];
    if (zones.length === 0)
        return [];
    const hits = [];
    for (const seg of segments(m)) {
        ...
        for (const z of zones) {
            if (z.kind !== 'quiet' || z.sourceId !== seg.sourceId)
                continue;
            if (Math.max(srcA, z.t0) < Math.min(srcB, z.t1) && !hits.some((h) => h.id === z.id))
                hits.push(z);
        }
    }
    return hits;
 ...
 * regression-safe.
 */
export function setTranscriptionGlossary(m, glossary) {
...
function clipAndShiftItems(items, t0, t1) {
    const out = [];
 ...
 */
export function sliceTimelineRange(m, a, b) {
    ...
    if (m.timeline.motion.length > 0)
        timeline.motion = clipAndShiftItems(m.timeline.motion, t0, t1);
    if (m.timeline.dialogue)
        timeline.dialogue = clipAndShiftItems(m.timeline.dialogue, t0, t1);
    if (m.timeline.music) {
        const music = [];
