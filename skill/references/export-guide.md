...
```bash
vedit export render final.mp4                     # captions.enabled なら字幕を既定で焼き込み
vedit export render final.mp4 --no-burn-captions   # 字幕なしのクリーン映像(NLEで字幕を仕上げる場合)
```

- 元素材(プロキシではない)から libx264 CRF18 でエンコード
- 全編エンコードはここが唯一。編集中は一切エンコードしない
- 字幕は `captions.enabled=true` かつ実際に焼けるcueがあれば既定で焼き込まれる
  (`--burn-captions` は後方互換の受理のみで no-op)。`--no-burn-captions` で
  opt-out すると字幕なしのクリーン映像になる — Resolve 側で字幕を自前で
  作り直す/仕上げたい人はこちらを使う。dialogue(セリフ)は captions 設定と
  無関係に常に焼き込まれる(レンダーが唯一の出口のため、opt-outできない)
