...
4. 判断に迷う箇所は `vedit view --from X --to Y` の PNG を Read して映像を確認

次の構造で報告(これ以外の長文は書かない):
- chapters: [{title, sourceId, おおよその範囲(語id), 一言要約}]
- highlights: [{語id範囲, 引用, 残すべき理由(フック/感情の山/見せ場)}]
- cutCandidates: [{ids: "w0120..w0134", sourceId, 引用, 理由(言い直し|脱線|だれ場|無音|フィラー), 秒数, confidence: high|low}]
- pauses: [{位置, 秒数, 判定: 意味のある間(残す)|だれ場(切る)|判断保留}]
...
5. 報告: 適用結果(rev 遷移、削減秒数合計)、view で見つけた違和感、
   caption の状態(/api/captions のクランプ超過や重複がないか)
リスト外の編集は一切しない。undo で戻せることを前提に、疑問があれば止まって報告。
```
