...
4. 文字起こし済みソースがあれば `vedit detect --project <dir>` を実行し候補を確認
   (注意: detect は候補を作成し、ユーザーの UI 提案タブにも現れる。
   副作用として報告に含めること)
5. 判断に迷う箇所は `vedit view --domain source --source <id> --from X --to Y` の
   PNG を Read(**transcript の時刻はソース時刻**。timeline ドメインに渡さない)
6. transcript が全体的に崩れているソース(誤認識だらけ、"?" 多発)は、
   その旨を risks に書き、`vedit scenes sheet` で視覚ベースの分析に切り替える
7. しゃべりのない素材(B-roll等)の keep/reject は `vedit scenes sheet` の
   サムネ格子を Read して各シーンに推薦する
8. キットがあれば `vedit kit` を読み、profile の spine/pacing を分析の評価軸に使う。
