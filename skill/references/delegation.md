...
2. 承認後: `vedit ingest-batch <対象> --project <dir> --language ja` で本実行
   (SHA-256 重複検出→スキップ、プロキシ生成+文字起こしは2本まで並行、
   進捗は stderr に出る)。プロジェクト外に元素材を集約したい場合のみ
   `--copy <destDir>` を付ける(既定は `--link` = 元パス参照のまま)
...
5. 最後に `vedit status` と各ソースの packed transcript 冒頭20行を報告

単発ファイル1本だけなら `vedit ingest <file> --language ja --project <dir>` で
このテンプレートを使わずインラインでよい。
コードの編集・git 操作は禁止。
```
...
7. キットがあれば `vedit kit` を読み、profile の spine/pacing を分析の評価軸に使う。
...
5. 適用後、主要な変更点を `vedit show range <t0> <t1>` で1箇所見せる
   (ユーザーは隣の Web UI 画面を見ながら会話している — revision は作らず --base も不要)
6. 報告: 適用結果(rev 遷移、削減秒数合計)、view で見つけた違和感、
